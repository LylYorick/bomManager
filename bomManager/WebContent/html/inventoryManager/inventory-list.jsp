...
<title>Insert title here</title>
	...
	<body>
		...
		<div class="text-c">
		<form class="Huiform" method="post" action="inventory-list" target="_self">
			<input type="text" class="input-text" style="width:250px" placeholder="输入料号（模糊）" id="" name="entity.id.partnumber">
			<input type="text" class="input-text" style="width:250px" placeholder="输入料号名称（模糊）" id="" name="entity.partName">
			<input type="text" class="input-text" style="width:250px" placeholder="供应商名称（模糊）" id="" name="entity.id.supplierName">
			<button type="submit" class="btn btn-success radius" id="" name=""><i class="Hui-iconfont">&#xe665;</i> 搜索库存信息</button>
		</form>
		</div>
		<div class="cl pd-5 bg-1 bk-gray mt-20">
		<span class="l">
		 	<a href="javascript:;" onclick="material_add('材料入库','invlog-toAdd.action','600','600')" class="btn btn-primary radius">
		 		<i class="Hui-iconfont">&#xe600;</i> 入库
			 </a>
		 	<a href="javascript:;" onclick="material_add('材料出库','invlog-toDelivery.action','600','600')" class="btn btn-primary radius">
		...
		<table class="table table-border table-bordered table-bg">
			<thead>
				<tr>
					<th scope="col" colspan="18">库存列表</th>
				</tr>
				<tr class="text-c">
					<th width="150">料号</th>
					<th width="100">版本</th>
			...
			<tbody>
				<s:iterator value="#request.InventoryList">
					<tr class="text-c">
						<td>${id.partnumber}</td>
						<td>${id.partRev}</td>
						...
						<td>${i_Qty}</td>
						<td>${m_Editor}</td>
						<td><s:date name="m_Datetime" format="yyyy-MM-dd hh:mm:ss"/></td>
					</tr>
				</s:iterator>
			</tbody>
		</table>
		...
		<form  id="fenyeForm" class="Huiform" method="post" action="inventory-list" target="_self">
